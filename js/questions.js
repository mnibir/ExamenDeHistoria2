...
window.onload = function(){ 
	...
	//xhttp.open("GET", "https://rawgit.com/mnibir/ExamenDeHistoria2/master/xml/correccion.xml", true);   
	xhttp.send();
	...
	//xhttp2.open("GET", "https://rawgit.com/mnibir/ExamenDeHistoria2/master/xml/correccion.xsl", true); 
	xhttp2.send();
	...
	// El antivirus está bloqueando esta función
	document.getElementById('boton5').onclick=function(){
  		window.alert("Entramos");
  	}
}
...
function darRespuestaHtml(r){
 ...
 p.appendChild(node);
 document.getElementById('informeResultados').appendChild(p);
}

function presentarNota(){
   document.getElementById('informeResultados').style.display = "block";
   //Código transformación xslt con xmlDoc y xslDoc
   if (document.implementation && document.implementation.createDocument) {
        xsltProcessor = new XSLTProcessor();
        xsltProcessor.importStylesheet(xslDoc);
        resultDocument = xsltProcessor.transformToFragment(xmlDoc, document);
        document.getElementById('informeResultados').appendChild(resultDocument);
   }
